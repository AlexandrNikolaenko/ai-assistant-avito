...
export default function ElectronicsOptionalFields({
  ...
  return (
    <Flex vertical gap={8} style={{ width: "100%", alignItems: "start" }}>
      <Select
        status={
          String((draft.params as ElectronicsParams).type ?? "") == ""
            ? "warning"
            : "validating"
        }
        style={{
            ...
            prev
              ? {
                  ...
                  params: {
                    ...
                    type: value as "phone" | "laptop" | "misc" | undefined,
      ...
      <Input
        status={
          String((draft.params as ElectronicsParams).brand ?? "") == ""
            ? "warning"
            : "validating"
        }
        style={{ width: "100%", maxWidth: "456px" }}
        placeholder="Бренд"
        value={String((draft.params as ElectronicsParams).brand ?? "")}
        onChange={(e) => setParamString("brand", e.target.value)}
      />
      <Input
        status={
          String((draft.params as ElectronicsParams).model ?? "") == ""
            ? "warning"
            : "validating"
        }
        style={{ width: "100%", maxWidth: "456px" }}
        placeholder="Модель"
        value={String((draft.params as ElectronicsParams).model ?? "")}
        onChange={(e) => setParamString("model", e.target.value)}
      />
      <Select
        status={
          String((draft.params as ElectronicsParams).condition ?? "") == ""
            ? "warning"
            : "validating"
        }
        style={{
        ...
        value={(draft.params as ElectronicsParams).condition ?? ""}
        options={[
          { value: "", label: "Не указано" },
          ...
          setDraft((prev) =>
            prev
              ? {
                  ...
                  params: {
                    ...(prev.params as Record<string, unknown>),
                    condition: value as "new" | "used" | undefined,
                  } as Partial<ItemParams>,
                }
              : prev,
          );
      ...
      <Input
        status={
          String((draft.params as ElectronicsParams).color ?? "") == ""
            ? "warning"
            : "validating"
        }
        style={{ width: "100%", maxWidth: "456px" }}
        placeholder="Цвет"
        value={String((draft.params as ElectronicsParams).color ?? "")}
