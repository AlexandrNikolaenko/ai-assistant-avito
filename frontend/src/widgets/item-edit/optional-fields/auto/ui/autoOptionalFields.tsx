...
export default function AutoOptionalFields({
  ...
  return (
    <Flex vertical gap={8} style={{ width: "100%", alignItems: "start" }}>
      ...
      <Input
        status={
          String((draft.params as AutoParams).yearOfManufacture ?? "") == ""
            ? "warning"
            : "validating"
        }
        style={{ width: "100%", maxWidth: "456px" }}
        ...
        type="number"
        value={
          (draft.params as AutoParams).yearOfManufacture === undefined
            ? ""
            : String((draft.params as AutoParams).yearOfManufacture)
        }
        onChange={(e) => setParamNumber("yearOfManufacture", e.target.value)}
        step={1}
      />
      <Select
        status={
          String((draft.params as AutoParams).transmission ?? "") == ""
            ? "warning"
            : "validating"
        }
        style={{
            ...
            prev
              ? {
                  ...
                  params: {
                    ...
                    transmission: value as "automatic" | "manual" | undefined,
      ...
      <Input
        ...
        onChange={(e) => setParamNumber("mileage", e.target.value)}
        step={1}
        min={0}
      />
      <Input
        ...
        onChange={(e) => setParamNumber("enginePower", e.target.value)}
        step={1}
        min={0}
      />
    </Flex>
